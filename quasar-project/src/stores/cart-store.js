...
export const userCartStore = defineStore("cart",{
    ...
    actions:{
        addToCart(pelicula){
            ...
            const existingPeliculaIndex = cartAllItems.findIndex(
                (item) => item.id == pelicula.id
            )

            if(existingPeliculaIndex>=0)
                cartAllItems[existingPeliculaIndex].quantity++;
            else
                cartAllItems.push({...pelicula, quantity: 1})
        ...
        removeToCart(peliculaId){
            ...
            const existingPeliculaIndex = cartAllItems.findIndex(
                (item) => item.id == peliculaId
            )

            if(existingPeliculaIndex>=0)
                cartAllItems.splice(existingPeliculaIndex,1)
        ...
        updateQuantity(peliculaId, newQuantity){
            ...
            if(existingPeliculaIndex>=0)
                cartAllItems[existingPeliculaIndex].quantity = newQuantity;
